<x-app-layout>
    <x-slot name="pageTitle">Edit BMI Assessment</x-slot>
    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-5">
            ...
            <div>
                <h2 class="text-xl font-bold text-gray-800">Edit BMI Assessment</h2>
                <p class="text-sm text-gray-500">Update your measurements for {{ \Carbon\Carbon::parse($record->assessed_date)->format('F Y') }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm px-5 py-3 mb-4 flex items-center gap-4 text-sm">
            ...
            <div class="ml-auto text-right">
                <p class="text-xs text-gray-400">Editing assessment for</p>
                <p class="font-semibold text-gray-700">{{ \Carbon\Carbon::parse($record->assessed_date)->format('F Y') }}</p>
            </div>
        </div>

        <form method="POST" action="{{ route('self-assessment.update') }}" id="bmiForm" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-5">
                ...
                <div class="lg:col-span-3 space-y-4">
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-5">
                        ...
                        <p class="text-xs text-gray-400 mb-4 ml-8">Update your measurements below</p>

                        <div class="grid grid-cols-2 gap-x-4 gap-y-8">

                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Height (cm) <span class="text-red-500">*</span></label>
                                <input type="number" name="height" id="height" value="{{ old('height', $record->height * 100) }}"
                            ...
                            <div>
                                ...
                                <input type="number" name="weight" id="weight" value="{{ old('weight', $record->weight) }}"
                            ...
                            <div>
                                ...
                                <input type="number" name="waist" id="waist" value="{{ old('waist', $record->waist) }}"
                            ...
                            <div>
                                ...
                                <input type="number" name="hip" id="hip" value="{{ old('hip', $record->hip) }}"
                            ...
                            <div>
                                ...
                                <input type="number" name="wrist" id="wrist" value="{{ old('wrist', $record->wrist) }}"
                    ...
                    <input type="hidden" name="bmi_value"         id="bmi_value_input"         value="{{ $record->bmi_value }}">
                    <input type="hidden" name="bmi_category"      id="bmi_category_input"      value="{{ $record->bmi_category }}">
                    <input type="hidden" name="weight_to_lose"    id="weight_to_lose_input"    value="{{ $record->weight_to_lose }}">
                    <input type="hidden" name="normal_weight_min" id="normal_weight_min_input" value="{{ $record->normal_weight_min }}">
                    <input type="hidden" name="normal_weight_max" id="normal_weight_max_input" value="{{ $record->normal_weight_max }}">
                    <input type="hidden" name="body_frame"        id="body_frame_input"        value="{{ $record->body_frame }}">
                    <input type="hidden" name="waist_hip_ratio"   id="waist_hip_ratio_input"   value="{{ $record->waist_hip_ratio }}">
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-5">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div x-data="photoCapture('photo_{{ $key }}', '{{ $record->{'photo_' . $key} ? asset('storage/' . $record->{'photo_' . $key}) : '' }}')" class="text-center">
                ...
                <div class="lg:col-span-2 lg:sticky lg:top-6 lg:self-start space-y-4">
                    ...
                    <!-- BMI Result (pre-filled) -->
                    <div class="bg-white rounded-xl shadow-sm p-5">
                        <div class="flex items-center gap-2 mb-4">
                            <span class="w-6 h-6 rounded-full bg-green-600 text-white text-xs font-bold flex items-center justify-center shrink-0" id="step2Badge">2</span>
                            <h3 class="font-semibold text-gray-800 text-sm">BMI Result</h3>
                        </div>

                        <div id="bmiResult">
                            <div class="text-center mb-5 pb-5 border-b border-gray-100">
                                <div id="bmiValue" class="text-5xl font-bold text-gray-800">{{ number_format($record->bmi_value, 2) }}</div>
                                <div id="bmiCategoryBadge" class="inline-block mt-2 px-4 py-1.5 rounded-full text-sm font-semibold
                                    @if($record->bmi_category === 'Underweight') bg-blue-100 text-blue-700
                                    @elseif($record->bmi_category === 'Normal') bg-green-100 text-green-700
                                    @elseif($record->bmi_category === 'Overweight') bg-yellow-100 text-yellow-700
                                    @elseif($record->bmi_category === 'Obese I') bg-orange-100 text-orange-700
                            ...
                            <div class="space-y-0 text-sm">
                                <div class="flex justify-between py-2.5 border-b border-gray-50">
                                    ...
                                    <span id="normalRange" class="font-semibold text-gray-800">{{ $record->normal_weight_min }} – {{ $record->normal_weight_max }} kg</span>
                                </div>
                                <div class="flex justify-between py-2.5 border-b border-gray-50">
                                    <span class="text-gray-500">Weight to Lose</span>
                                    <span id="weightToLose" class="font-semibold text-gray-800">{{ $record->weight_to_lose ? $record->weight_to_lose . ' kg' : '—' }}</span>
                                </div>
                                <div class="flex justify-between py-2.5 border-b border-gray-50">
                                    <span class="text-gray-500">Body Frame</span>
                                    <span id="bodyFrame" class="font-semibold text-gray-800">{{ $record->body_frame ?: '—' }}</span>
                                </div>
                                <div class="flex justify-between py-2.5">
                                    <span class="text-gray-500">Waist-Hip Ratio</span>
                                    <span id="waistHipRatio" class="font-semibold text-gray-800">{{ $record->waist_hip_ratio ? number_format($record->waist_hip_ratio, 2) : '—' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function photoCapture(fieldName, existingUrl) {
            return {
